...
class DatastaxDriver extends EventEmitter {
  ...
  _getExistingPool(keyspace, config, waitForConnect, callback) {
    if (keyspace) {
      config = _.extend(config, { keyspace: keyspace });
    }
    var pool = this.pools[keyspace];
    ...
    if (!pool || pool.isClosed || !_.isEqual(pool.storeConfig, config)) {
      var createConnection = this._createConnectionPool.bind(this, config, waitForConnect, (err, pool) => {
        if (err) {
          return void callback(err);
        }
      ...
      if (pool && !pool.isClosed) {
        return void this._closePool(pool, createConnection);
      }
