...
class DatastaxDriver extends EventEmitter {
  ...
  _getConnectionPool(keyspace, waitForConnect, callback) {
    ...
    const getExistingPool = (config, cb) => {
      if (keyspace) {
    ...
    var poolConfig = _.extend(emptyConfig, this.poolConfig);
    if (this.connectionResolver) {
      var resolutionRequestId = uuid.v4();
      this.emit('connectionResolving', resolutionRequestId);
      this.connectionResolver.resolveConnection(this.config, (err, connectionData) => {
        if (err) {
