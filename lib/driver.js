...
class DatastaxDriver extends EventEmitter {
  ...
  async _executeCqlOnDriver(pool, cqlStatement, params, consistency, options) {
    ...
    params = opts.params;
    try {
      const data = await pool.execute(cqlStatement, params, execOptions);
  ...
  _execCqlStream(cql, dataParams, options, stream) {
    return this._execOnPoolReady(options, (e, pool, consistency) => {
      if (e) {
  ...
  _execCql(cql, dataParams, options, queryRequestId) {
    return this._execOnPoolReady(options, async (e, pool, consistency) => {
      if (e) {
        throw e;
      }
      ...
      try {
        let results = await this._executeCqlOnDriver(pool, cql, dataCopy, consistency, options);
  ...
  async _execOnPoolReady(options, task) {
    ...
    try {
      ...
      if (pool.isReady) {
        return task(null, pool, consistency);
      }
      return new Promise(resolve => {
        pool.waiters.push(e => resolve(task(e, pool, consistency)));
      });
    } catch (err) {
      return task(err);
    }
  ...
  _execCqlWithRetries(cqlQuery, dataParams, options) {
    ...
    return new Promise((resolve, reject) => {
      const executeRetryableCql = () => {
        operation.attempt(async currentAttempt => {
          ...
          try {
            let result = await this._execCql(cqlQuery, dataParams, options, queryRequestId);
  ...
  */
  async cql(cqlQuery, dataParams, options, callback) {
    ...
    try {
      const result = await this._execCqlWithRetries(cqlQuery, dataParams, options);
      return callback ? void callback(null, result) : result;
    } catch (err) {
      if (callback) {
  ...
  */
  async namedQuery(name, dataParams, options, callback) {
    ({ options, callback } = checkOptionalParameters(options, callback));
    ...
    try {
      ...
      options.executeAsPrepared = this.config.supportsPreparedStatements &&
      (typeof options.executeAsPrepared === 'undefined') ? true : !!options.executeAsPrepared;
      const rows = await this.cql(queryText, dataParams, options);
      return callback ? void callback(null, rows) : rows;
    } catch (err) {
      err.query = {
        queryName: name,
        params: dataParams,
