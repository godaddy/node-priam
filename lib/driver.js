...
class DatastaxDriver extends EventEmitter {
  ...
  async _getConnectionPool(keyspace, waitForConnect, callback) {
    ...
    this._getExistingPool(keyspace, poolConfig, waitForConnect, callback);
  }

  async _getExistingPool(keyspace, config, waitForConnect, callback) {
    if (keyspace) {
      config = _.extend(config, { keyspace: keyspace });
    }
    const pool = this.pools[keyspace];
    ...
    if (!pool || pool.isClosed || !_.isEqual(pool.storeConfig, config)) {
      const createConnection = this._createConnectionPool.bind(this, config, waitForConnect, (err, pool) => {
        if (err) {
          return void callback(err);
        }
  ...
  async _createConnectionPool(poolConfig, waitForConnect, callback) {
    ...
    this._performConnect(keyspace, false, (err, pool) => {
      ...
      if (pool.isReady === false) {
        pool.waiters.push(e => {
          ...
          this._execCql(cql, dataParams, options, queryRequestId, callback);
        });
        return;
      }
