...
var cqlDriverNumericTypes = _.pick(cqlDriver.types, ['BigDecimal', 'Integer', 'Long']);
var cqlDriverStringTypes = _.pick(cqlDriver.types, ['Uuid', 'TimeUuid']);

function shouldCoerceDataStaxTypes(options, config) {
  var coerce = (options || {}).coerceDataStaxTypes;
  if (coerce === undefined || coerce === null) {
...
function coerceDataStaxTypes(record) {
  _.forIn(record, function (value, key) {
    if (typeof value !== 'object') { return; } // already a primitive, or null
    _.forOwn(cqlDriverNumericTypes, function (type) {
      if (value instanceof type) {
        record[key] = value instanceof cqlDriverNumericTypes.BigDecimal ? parseFloat(value.toString()) : parseInt(value.toString(), 10);
      }
    });
    _.forOwn(cqlDriverStringTypes, function (type) {
      if (value instanceof type) {
