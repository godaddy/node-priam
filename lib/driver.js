...
class DatastaxDriver extends EventEmitter {
  ...
  async _createConnectionPool(poolConfig, waitForConnect) {
    ...
    const connect = async () => {
      try {
        await pool.connect();
        pool.isReady = true;
        this.emit('connectionOpened', openRequestId, pool);
  ...
  async _closePool(pool) {
    ...
    this.emit('connectionClosed', pool);
  }
  ...
  _transformRecord(record, options) {
    ...
    if (Array.isArray(options.resultTransformers)) {
      for (let i = 0; i < options.resultTransformers.length; i++) {
  ...
  async _timeQuery(queryRequestId, cqlQuery, dataParams, options, act) {
    try {
      ...
      this.emit('queryFailed', queryRequestId, err);
      throw err;
    }
  }
  ...
  _checkObjectResult(result, fieldName, options) {
    ...
    if (options && options.resultHint && options.resultHint[fieldName]) {
      const type = options.resultHint[fieldName];
  ...
  _getNormalizedRecord(row, options) {
    const shouldCoerce = shouldCoerceDataStaxTypes(options, this.config);
    const result = {};
    _.forOwn(row, (value, name) => {
      if ((name === 'columns' || name === '__columns') && _.isObject(value)) {
        return;
      } // skip metadata
      if (typeof value === 'string') {
        value = this._checkObjectResult(value, name, options);
      }
      result[name] = value;
    });
    if (shouldCoerce) {
  ...
  */
  ...
  select(cql, dataParams, options, callback) {
    const params = setDefaultConsistency(this.consistencyLevel.ONE, options, callback);
    this.cql(cql, dataParams, params.options, params.callback);
  }

  insert(cql, dataParams, options, callback) {
    const params = setDefaultConsistency(this.consistencyLevel.LOCAL_QUORUM, options, callback);
    this.cql(cql, dataParams, params.options, params.callback);
  }
  ...
  beginQuery() {
    return new Query(this, {
      options: { consistency: this.poolConfig.consistencyLevel },
      resultTransformers: this.resultTransformers.slice(0)
    });
  }
...
function setDefaultConsistency(defaultConsistencyLevel, options, callback) {
  const p = checkOptionalParameters(options, callback);
...
function shouldCoerceDataStaxTypes(options, config) {
  let coerce = options.coerceDataStaxTypes;
...
function changePorts(hostsArray, from, to) {
  return hostsArray.map(function (host) {
    const ix = host.indexOf(':');
    if (ix === -1) {
      return host;
    }

    const hostName = host.substring(0, ix);
    let port = host.substring(ix + 1);
