...
class DatastaxDriver extends EventEmitter {
  ...
  async _execCqlStream(cql, dataParams, options, stream) {
    try {
      return await this._execWithRetries(cql, dataParams, options, (pool, consistency) => {
        ...
        return this._streamCqlOnDriver(pool, cql, dataCopy, consistency, options, stream);
      });
    } catch (err) {
  ...
  async _execCql(cql, dataParams, options) {
    return this._execWithRetries(cql, dataParams, options, (pool, consistency) => {
      // client removes used data entries, so we need to create a copy of the data in case it's needed for retry...
      const dataCopy = dataParams.slice();
    ...
    return new Promise((resolve, reject) => {
      const executeRetryableCql = () => {
        operation.attempt(async (currentAttempt) => {
          ...
          this.emit('queryStarted', queryRequestId, cqlQuery, dataParams, consistency, options);
          try {
            options.consistency = consistency;
            const { pool } = await this._preparePool(options);
            const result = await task(pool, consistency, queryRequestId);
  ...
  */
  async cql(cqlQuery, dataParams, options, callback) {
    ...
    try {
      const result = await this._execCql(cqlQuery, dataParams, options);
