...
class DatastaxDriver extends EventEmitter {
  ...
  async _execCqlStream(cql, dataParams, options, stream) {
    try {
      const { pool, consistency } = await this._preparePool(options);
      await this._streamCqlOnDriver(pool, cql, dataParams, consistency, options, stream);
    } catch (err) {
      return void stream.emit(err);
    }
  }

  async _execCql(cql, dataParams, options, queryRequestId) {
    const { pool, consistency } = await this._preparePool(options);
  ...
  async _preparePool(options) {
    ...
    return { pool, keyspace, consistency };
  }
          ...
          try {
            ...
            if (this._canRetryError(err)) {
              ...
              if (enableConsistencyFailover && (consistency in retryConsistencyDownlevels)) {
                options.consistency = consistency = retryConsistencyDownlevels[consistency];
