...
class DatastaxDriver extends EventEmitter {
  ...
  initConnectionResolver(context) {
  ...
  getConnectionPool(keyspace, waitForConnect, callback) {
    var self = this;
  ...
  createConnectionPool(poolConfig, waitForConnect, callback) {
    var self          = this,
  ...
  remapConnectionOptions(connectionData) {
  ...
  connect(keyspace, callback) {
    ...
    this.performConnect(keyspace, true, callback);
  }

  performConnect(keyspace, waitForConnect, callback) {
  ...
  connectionResolverFetchHandler(err, data) {
    if (err) {
  ...
  transformRecord(record, options) {
    if (record) {
      var normalized = this.getNormalizedResults([record]);
      record = normalized[0];
  ...
  executeCqlOnDriver(pool, cqlStatement, params, consistency, options, callback) {
  ...
  streamCqlOnDriver(pool, cqlStatement, params, consistency, options, stream) {
    Readable
      .from(this.iterateCqlOnDriver(pool, cqlStatement, params, consistency, options))
      .pipe(stream);
  }

  async *iterateCqlOnDriver(pool, cqlStatement, params, consistency, options) {
    const opts = this.prepareQueryArgs(pool, cqlStatement, params, consistency, options);
    const execOptions = opts.execOptions;
    cqlStatement = opts.cqlStatement;
    params = opts.params;
    try {
      for await (const row of pool.stream(cqlStatement, params, execOptions)) {
        yield this.transformRecord(row, execOptions);
      }
    } catch (err) {
      err.query = {
        cql: cqlStatement,
  ...
  execCqlStream(cql, dataParams, options, stream) {
    ...
    this.performConnect(keyspace, false, function (err, pool) {
      if (err) {
  ...
  execCql(cql, dataParams, options, queryRequestId, callback) {
    /* istanbul ignore next: no benefit of testing coalesce for keyspace retrieval */
    var self        = this,
      keyspace    = (options || {}).keyspace || this.keyspace,
      consistency = options.consistency || self.poolConfig.consistencyLevel || self.consistencyLevel.one;
  ...
  */
  cql(cqlQuery, dataParams, options, callback) {
    ...
    dataParams = this.normalizeParameters(dataParams);

    // Keep passing the stream down the chain so we are still reusing logic
    if (isStream(callback)) {
      return this.execCqlStream(cqlQuery, dataParams, options, callback);
    }
    return this.execCqlWithRetries(cqlQuery, dataParams, options, callback);
  }

  execCqlWithRetries(cqlQuery, dataParams, options, callback) {
    ...
    const executeRetryableCql = () => {
      operation.attempt(currentAttempt => {
        ...
        this.execCql(cqlQuery, dataParams, options, queryRequestId, (err, result) => {
          const canRetryErrorType = !!err && this.canRetryError(err);
  ...
  prepareQueryArgs(pool, cqlStatement, params, consistency, options) {
    var self = this;
  ...
  checkObjectResult(result, fieldName, options) {
  ...
  getNormalizedResults(original, options) {
    var self = this;
