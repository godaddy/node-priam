...
Batch.prototype.add = function add(queryOrBatch) {
  ...
  } else if (queryOrBatch instanceof Query) {
    this.addQuery(queryOrBatch);
  } else if (queryOrBatch instanceof Batch) {
...
function joinQueries(queries, timestamp, batchType, consistencyHierarchy, cqlVersion, hasChildBatch, isChildBatch) {
  var cql = []
    ...
    , hasTimestamp = false
    , hasQuery = false
    , supportsNestedTimestamp = supportsCql31(cqlVersion)
    , queryConsistencyMap;
  if (!isChildBatch) {
  ...
  if (timestamp !== undefined && timestamp !== null) {
    ...
    if ((!supportsNestedTimestamp && !isChildBatch) || (!isChildBatch && !hasChildBatch)) {
      ...
      params.unshift({ value: timestamp, hint: 'int' });
    }
  }
  _.each(queries, function (query) {
    ...
    if (query instanceof Batch) {
      ...
      if (queryCql && endsWithSemicolonRegex.test(queryCql)) {
    ...
    if (queryCql) {
      if (!isBatch) {
        if (hasTimestamp && supportsNestedTimestamp && (isChildBatch || hasChildBatch)) {
          queryCql += '\nUSING TIMESTAMP ?';
          params.push({ value: timestamp, hint: 'int' });
        }
        queryCql += ';';
      }
      cql.push(queryCql);
...
function executeBatch(callback) {
  ...
  if (!self.context.queries.length) {
    self.context.errors.push('#add(), #addQuery(), or #addBatch() should be called prior to #execute()');
