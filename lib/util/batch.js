...
function joinQueries(queries, timestamp, consistencyHierarchy) {
...
function executeBatch(callback) {
    ...
    joined = joinQueries(self.context.queries, self.context.timestamp, self.context.consistencyHierarchy);
