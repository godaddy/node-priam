...
HelenusDriver.prototype.streamCqlOnDriver = function streamCqlOnDriver(pool, cqlStatement, params, consistency, options, stream) {
  var self = this;

  // Helenus doesn't support streaming data back from Cassandra, so just execute the results as normal and
  // then chunk the data back to the caller. Unfortunately, the entire data set will still be in memory.
  pool.cql(cqlStatement, params, function (err, data) {
    if (err) {
      stream.emit('error', err);
      return void stream.emit('end');
    }

    if (!Array.isArray(data)) { data = []; }
    var transformStream = self.getResultTransformStream(options)
      .on('error', stream.emit.bind(stream, 'error'))
      .pipe(stream);

    async.eachSeries(data, function arrayStreamReading(row, next) {
      var err = null;
      try {
    ...
    }, function arrayStreamEnd(err) {
      process.nextTick(function arrayStreamEndTick() {
        if (err) {
