...
For expected large data sets, it is a good idea to stream the data back.
```javascript
db
  .beginQuery()
  .query('SELECT "myCol1", "myCol2" FROM "myColumnFamily" WHERE "keyCol1" = ? AND "keyCol2" = ?')
  ...
  .on('end', function() {
    // fires when all rows have returned
  });
```
...
Release Notes
-------------
 - `1.1.0`: Adds streaming support with `Query.stream()` or `db.cql()`
